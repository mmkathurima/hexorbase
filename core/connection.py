...
class connection_setting_dialog(QtWidgets.QDialog,setting_dialog):
    ...
    def __init__(self):
        ...
        #
        self.server_port_check.clicked.connect(self.port_selected)
        self.buttonBox.accepted.connect(self.initiate_connection)
    ...
    def initiate_connection(self):
        ...
        if database_server_hostname == '':
            QtWidgets.QMessageBox.warning(self,"Invalid Connection Details","Please input a valid Hostname or IP address of the Database Server")
            reexecute_dialog.exec_()
        else:
            if self.server_port_check.isChecked() == True:
                if database_server_port.isdigit() == False:
                    QtWidgets.QMessageBox.warning(self,"Invalid Connection Details","Please input a valid Port Number of which the Database Server runs")
                    reexecute_dialog.exec_()
            ...
            if variables.database_type == 'MySQL':                                            # MySQL Database Connection is initiated here
                ...
                try:
                    ...
                    if int(exception.args[0]) == 1045:
                        QtWidgets.QMessageBox.warning(self,"Invalid Username or Password","Access denied for user '%s' on server '%s'"%(variables.username,variables.server_connection))
                        variables.oracle_status.setText("<font color=red>Connection Failed</font>")
                    elif int(exception.args[0]) == 2005:
                        QtWidgets.QMessageBox.warning(self,"Connection Failed","Unknown MySQL server host '%s'"%(variables.server_connection))
                        variables.oracle_status.setText("<font color=red>Connection Failed</font>")
                        reexecute_dialog.exec_()
                    elif int(exception.args[0]) == 2002:
                        QtWidgets.QMessageBox.warning(self,"Connection Failed","Can't connect to MySQL server through socket, connection timeout")
                        variables.oracle_status.setText("<font color=red>Connection Failed</font>")
                        reexecute_dialog.exec_()
                    else:
                        QtWidgets.QMessageBox.warning(self,"Connection Failed","%s : %s"%(exception.args[0],exception.args[1]))
                        variables.oracle_status.setText("<font color=red>Connection Failed</font>")
                        reexecute_dialog.exec_()
                ...
                try:
                    ...
                    if  'ORA-01017' in str(exception):
                        QtWidgets.QMessageBox.warning(self,"Connection Failed","%s"%(exception))
                        variables.mysql_status.setText("<font color=red>Connection Failed</font>")
                    elif 'ORA-12560' in str(exception):
                        QtWidgets.QMessageBox.warning(self,"Connection Failed","Can't connect to Oracle server through socket, connection timeout")
                        variables.mysql_status.setText("<font color=red>Connection Failed</font>")
                        reexecute_dialog.exec_()
                    else:
                        QtWidgets.QMessageBox.warning(self,"Connection Failed","%s"%(exception))
                ...
                try:
                    ...
                    self.database_interaction()                    # Run database interaction dialog
                except (pymssql.OperationalError) as exception:
                    if 'Login failed for user' in str(exception):
                        QtWidgets.QMessageBox.warning(self,"Connection Failed","Login failed for user %s"%(variables.username))
                        ...
                        QtWidgets.QMessageBox.warning(self,"Connection Failed","Can't connect to Microsoft SQL server through socket, connection timeout")
                        variables.sqlite_label.setText("<font color=red>Connection Failed</font>")
                        reexecute_dialog.exec_()
                    else:
                        QtWidgets.QMessageBox.warning(self,"Connection Failed","%s"%(exception))
    ...
    def database_interaction(self):
        ...
        try:
            ...
            QtWidgets.QMessageBox.warning(self,'SQL Syntax API not installed','SQL Code editor API is not installed,Please run <font color=blue> apt-get install python-qscintilla2</font> from linux terminal to get the API installed.')
