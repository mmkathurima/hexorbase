...
import _thread       # for bruteforce attack
...
from PyQt5 import QtCore      # GUI Library, QtGui, QtWidgets
...
class main_window(QtWidgets.QMainWindow,Ui_MainWindow):
    ...
    def __init__(self):
        ...
        #
        self.mysql_button.clicked.connect(self.mysql_settings)
        self.oracle_button.clicked.connect(self.oracle_settings)
        self.postgresql.clicked.connect(self.postgresql_settings)
        self.sql_button.clicked.connect(self.mssql_settings)
        self.sqlite_button.clicked.connect(self.sqlite_settings)
        self.login_button.clicked.connect(self.lock_login_area)
        self.password_manager_button.clicked.connect(self.launch_password_manager)
        self.brutefore_database.clicked.connect(self.launch_bruteforce)
    ...
    def database_interaction(self):
        ...
        try:
            ...
            QMessageBox.warning(self,'SQL Syntax API not installed','SQL Code editor \
    ...
    def lock_login_area(self):
        if self.username_linedit.text() == '':
            ...
            QtWidgets.QMessageBox.warning(self,"Null Credential","Please input a username before locking the login button")
    ...
    def mysql_settings(self):                       # MYSQL SETTINGS
        ...
        if mysql_install_status != 'Not Installed':
            ...
            if variables.username == '':
                QtWidgets.QMessageBox.warning(self,"Username error","Please Input a username and press the lock button to enable Generic database mode")
                self.username_linedit.setFocus()
            elif self.login_button.isChecked() == False:
                QtWidgets.QMessageBox.warning(self,"Lock Button","Please press the Lock Button to enable Generic database mode")
...
class database_interaction_dialog(QtWidgets.QDialog,interaction_dialog):
    ...
    def __init__(self):
        ...
        self.execute_sql_button.clicked.connect(self.browse_sql_script)
        self.close_interact_button.clicked.connect(self.close_database)
        self.tables_button.clicked.connect(self.show_tables)
        self.interact_browse.clicked.connect(self.connect_sqlite_database)
    ...
    def save_report(self):
        ...
        variables.report_save_path = QtWidgets.QFileDialog.getSaveFileName(self,"Save Reports","hexorbase-report-%s"%(number),"HTML(*.html)")[0]
    ...
    def connect_sqlite_database(self):
        ...
        database_name = str(QtWidgets.QFileDialog.getOpenFileName(self,"Select SQlite Database",""))[0]
        if database_name:
    ...
    def browse_sql_script(self):
        ...
        '''
        try:
            sql_script = QtWidgets.QFileDialog.getOpenFileName(self,"Select SQL Script","","SQL Script(*.sql)")[0]
    ...
    def table_insert_data(self,col_data,row_data):
        ...
        for row in range(len(row_data)):
            ...
            data_ = row_data[row]
            for row_number in range(len(column_data)):
...
class api_install_link(QtWidgets.QDialog,api_install_reference):
